...
Windows 用户可以使用 [Lyricify Lite](https://docs.lyricify.app/lyricify-lite/getting-started/) 显示桌面歌词、任务栏歌词和灵动词岛。

> [!IMPORTANT]
> 需要满足以下条件：
> 1. 使用 Windows 10 1809 及以上版本，并安装 `.NET Desktop Runtime 8.0`
> 2. 下载和安装 [Lyricify Lite](https://lyricify.app/download/)
> 3. 确认配置文件中 `main.lyric.desktop.enable = true`
> 4. 如需 go-musicfox 启动时自动拉起 Lyricify Lite，设置 `main.lyric.desktop.autoStart = true`；自动查找失败时可在 `main.lyric.desktop.command` 填写 Lyricify Lite 的绝对路径
> 5. go-musicfox 会通过 Windows System Media Transport Controls（SMTC）上报歌曲信息和播放进度，启动 Lyricify Lite 后播放歌曲即可识别
## 安装
